...
<div class="design-view row mb60">
    ...
    <?php
    if(is_dir("images/design/".$model->id)){
        if($model->main_img){
            $img=Html::img("@web/images/design/".$model->id."/".$model->main_img, ['class'=>'img-responsive design-img js_img', 'alt'=>'']);
            echo Html::a($img,'#',['class'=>'js_photo_swipe','data-index'=>0]);
        }
        $imgs=scandir("images/design/".$model->id);
        foreach($imgs as $key=>$img){
            $key--;
            if($img!='.' && $img!='..' && $img!=$model->main_img && $img!='s_'.$model->main_img){
                if(strpos($img,'s_' )=== false)
                {
                    $img=Html::img("@web/images/design/".$model->id."/".$img, ['class'=>'img-responsive design-img js_img', 'alt'=>'']);
